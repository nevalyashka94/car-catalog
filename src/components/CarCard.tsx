...
export default function CarCard({ car }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Текстовый контент: только Марка, Модель, Кузов и Цена */}
      <div className="flex flex-1 flex-col justify-between p-7">
        ...
        {open && (
          <div className="mt-4 rounded-2xl border border-white/10 bg-[#06080d] p-4">
            ...
            {loading ? (
              <div className="py-2 text-xs text-slate-400">Загрузка...</div>
            ) : dealers.length > 0 ? (
              <div className="space-y-2">
                {dealers.map((dealer) => (
                  <div
                    key={dealer.id}
                    className="rounded-xl border border-white/5 bg-[#121826] px-3.5 py-2 text-xs text-slate-200"
                  >
                    🏢 {dealer.name}
                  </div>
                ))}
              ...
              <div className="text-xs text-slate-500">Дилеры не найдены</div>
            )}
          </div>
        )}
      </div>
